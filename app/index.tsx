...
// HeaderProps moved/removed (not used in this file)
...
const CalendarView: React.FC<CalendarViewProps> = ({
  ...
  // handleSelectTime removed (duplicate)
      ...
      >
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          ...
          <View style={{ flexDirection: 'row', gap: 4, backgroundColor: colors.backgroundTertiary, borderRadius: 8, padding: 2 }}>
              ...
              >
                <ThemedText style={{ fontSize: 12, fontWeight: '600', color: distributionMode === 'category' ? colors.text : colors.textTertiary }}>{t('analytics.byCategory')}</ThemedText>
              </Button>
              <Button
              ...
              >
                <ThemedText style={{ fontSize: 12, fontWeight: '600', color: distributionMode === 'project' ? colors.text : colors.textTertiary }}>{t('analytics.byProject')}</ThemedText>
              </Button>
          </View>
        </View>
              ...
              <View style={{ paddingHorizontal: 24, marginBottom: 20 }}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <Button
                    onPress={() => setModalOpen(false)}
                    variant="ghost"
                    style={{ width: 32, height: 32, borderRadius: 16, backgroundColor: colors.backgroundTertiary, justifyContent: 'center', alignItems: 'center' }}
                  >
                          ...
                          <View style={{ flex: 1, backgroundColor: colors.backgroundSecondary, padding: 12, borderRadius: 12, gap: 8 }}>
                            ...
                            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
                              <Button onPress={() => setEditingProject({ ...editingProject, category: null, hexColor: '#9CA3AF' })} variant={editingProject.category ? 'ghost' : 'secondary'} style={{ paddingHorizontal: 12, paddingVertical: 8, borderRadius: 8, backgroundColor: !editingProject.category ? colors.text : colors.surface, borderWidth: 1, borderColor: !editingProject.category ? colors.text : colors.border }}>
                                <ThemedText style={{ fontSize: 12, fontWeight: '600', color: !editingProject.category ? colors.textInverse : colors.textTertiary }}>{t('projects.uncategorized')}</ThemedText>
                              </Button>
                              {Object.entries(categories).map(([catName, catColor]) => {
                                ...
                                return (
                                  <Button key={catName} onPress={() => setEditingProject({ ...editingProject, category: catName, hexColor: catColor })} variant={isSelected ? 'secondary' : 'ghost'} style={{ paddingHorizontal: 12, paddingVertical: 8, borderRadius: 8, backgroundColor: isSelected ? catColor : colors.surface, borderWidth: 1, borderColor: catColor }}>
                                    <ThemedText style={{ fontSize: 12, fontWeight: '600', color: isSelected ? colors.accentText : catColor }}>{t(`categories.${catName.toLowerCase()}`, { defaultValue: catName })}</ThemedText>
                                  </Button>
                                );
                              })}
                            </View>
