      ...
      <View style={[styles.calendarDropdown, { backgroundColor: colors.surface, shadowColor: colors.text }]}>
        ...
        <View style={styles.calendarWeekdays}>
          {['sun', 'mon', 'tue', 'wed', 'thu', 'fri', 'sat'].map((day) => (
            <Text key={day} style={[styles.calendarWeekdayLabel, { color: colors.textTertiary }]}>
              {t(`calendar.${day}`)}
            </Text>
          ))}
        </View>
        ...
        <View style={styles.calendarGrid}>
          ...
          {calendarDays.map((day) => {
            ...
            const dayDateStr = `${selectedDate.getFullYear()}-${String(
            ...
            if (hasEvents) {
              ...
              const mostCommonColor = Object.entries(colorCount).sort(
                (a, b) => b[1] - a[1]
              )[0]?.[0];
              if (mostCommonColor) {
